...
bool loadGlyph(Shape &output, FontHandle *font, int unicode, double *advance) {
    ...
    for (int i = 0; i < font->face->glyph->outline.n_contours; ++i) {
		...
		for ( int round = 0, index = first; round == 0; ++index )
		{
			...
			Point2 point( font->face->glyph->outline.points[ index ].x / 64.,
						  font->face->glyph->outline.points[ index ].y / 64. );
			PointType pointType = font->face->glyph->outline.tags[ index ] & 1 ? PATH_POINT :
								  font->face->glyph->outline.tags[ index ] & 2 ? CUBIC_POINT : QUADRATIC_POINT;
			...
			switch ( state )
			{
				...
				REQUIRE( pointType != QUADRATIC_POINT );
					...
					} else
					{
						...
						contour.addEdge(
								new CubicSegment( startPoint, controlPoint[ 0 ], controlPoint[ 1 ], midPoint ));
						startPoint = midPoint;
						controlPoint[ 0 ] = point;
					}
					state = pointType;
