...
namespace RetroUi
{
	...
	class SystemMain : public RetroEcs::ISystem
	{
		...
		shared_ptr<RetroResource::BitmapLoader> _bitmap_loader;
	...
	public:
		explicit SystemMain(
				shared_ptr<RetroResource::BitmapLoader> bitmap_loader,
				...
				: _bitmap_loader( bitmap_loader ), _package_loader( package_loader )
			, _packages( packages )
			...
			, _c_transform( c_transform )
		{
			...
			_main = _entity_factory->create_grid( _ui->scene, glm::vec3( -800.f, 400.f, 0.f ), glm::ivec2( 10 ),
												  glm::ivec2( 166 ));
...
//			}
			...
			{
				pt::ptree model;
				pt::read_xml( "res/gamelist.xml", model );
				...
				for ( auto& m : model.get_child( "gameList" ))
				{
					...
					const auto& image_path = m.second.get<std::string>( "image" );
					_bitmap_loader->load( &image_path, &image_path, &handle, 1 );
					_texture_manager->load( &handle );
				}

				_entity_factory->create_view( _main, "boxart", model );
			}
